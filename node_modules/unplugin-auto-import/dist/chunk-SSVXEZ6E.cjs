...
var _chunkGOGBRDVIcjs = require('./chunk-GOGBRDVI.cjs');
...
function normalizeImport(info, name) {
  ...
  return _chunkGOGBRDVIcjs.__spreadValues.call(void 0, {
    name,
    as: name
  }, info);
}
...
function createContext(options = {}, root = process.cwd()) {
  ...
  const cachePath = isCache === false ? false : _path.resolve.call(void 0, root, typeof isCache === "string" ? isCache : "node_modules/.cache/unplugin-auto-import.json");
...
`;
  ...
  async function getCacheData(cache) {
    const str = (await _fs.promises.readFile(cache, "utf-8")).trim();
  ...
  async function generateCache() {
    ...
    try {
      ...
      await Promise.allSettled(Object.keys(cacheData).map(async (filePath) => {
        try {
          const normalizeRoot = root.replaceAll(_path.sep, _path.posix.sep);
          await _fs.promises.access(_path.posix.join(normalizeRoot, filePath));
        } catch (e) {
  ...
  async function updateCacheImports(id, importList) {
    ...
    await unimport.modifyDynamicImports(async (imports) => {
      const cacheData = await getCacheData(cachePath);
      if (id && importList) {
        const filePath = _path.posix.normalize(_path.posix.relative(root, id));
...
async function flattenImports(map, overriding = false) {
  const promises = await Promise.all(_utils.toArray.call(void 0, map).map(async (definition) => {
    if (typeof definition === "string") {
      if (!_chunkGOGBRDVIcjs.presets[definition])
        throw new Error(`[auto-import] preset ${definition} not found`);
      const preset = _chunkGOGBRDVIcjs.presets[definition];
  ...
  return {
    ...
    vite: {
      async handleHotUpdate({ file }) {
        var _a;
        if ((_a = ctx.dirs) == null ? void 0 : _a.some((glob) => _minimatch2.default.call(void 0, _utils.slash.call(void 0, file), glob)))
          await ctx.scanDirs();
      },
