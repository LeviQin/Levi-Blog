<template>
  <div class="pw-card">
    <div class="pw-card-head">
      <div class="pw-algo-badge">DES</div>
      <div>
        <div class="pw-card-title">DES 加/解密</div>
        <div class="pw-card-sub">Data Encryption Standard &mdash; 历史悠久的对称加密标准</div>
      </div>
    </div>
    <div class="pw-toolbar">
      <button class="pw-btn pw-btn-encrypt" @click="encrypt"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>加密</button>
      <button class="pw-btn pw-btn-decrypt" @click="decrypt"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0110 0v4"/><line x1="12" y1="17" x2="12" y2="19"/></svg>解密</button>
      <span class="pw-toolbar-spacer"></span>
      <button class="pw-btn pw-btn-clear" @click="clear"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>清除</button>
    </div>
    <div class="pw-key-row">
      <div class="pw-key-input-wrap">
        <el-input v-model="secretKey" placeholder="输入密钥（可选）" size="default" clearable>
          <template #prefix><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="opacity:.5"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 11-7.778 7.778 5.5 5.5 0 017.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/></svg></template>
        </el-input>
      </div>
    ...
    <div class="pw-io-grid">
      ...
      <div class="pw-io-arrow"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg></div>
      <div class="pw-io-block">
    ...
    <div class="pw-info-section">
      <div class="pw-info-head" @click="infoOpen = !infoOpen">
        <svg class="icon" aria-hidden="true"><use xlink:href="#levi-guanyu-"></use></svg><span>关于 DES</span>
      ...
      <div class="pw-info-body" v-show="infoOpen">
        <p>在线DES加密，DES解密，可以输入密钥后进行加密和解密，支持可逆加密解密。</p>
        <p>数据加密标准（Data Encryption Standard，DES）是一种使用密钥加密的块密码，1976年被美国联邦政府的国家标准局确定为联邦资料处理标准，随后在国际上广泛流传开来。</p>
        <dl class="pw-info-dl">
          <div><dt>DES全称</dt><dd>Data Encryption Standard</dd></div>
          <div><dt>特点</dt><dd>对称密钥，56位有效密钥</dd></div>
          <div><dt>类型</dt><dd>对称加密</dd></div>
          <div><dt>分组长度</dt><dd>64位</dd></div>
          <div><dt>提出时间</dt><dd>1976年</dd></div>
        </dl>
      </div>
    </div>
  </div>
...
const secretKey = ref(""), textarea = ref(""), result = ref("");
const copyDone = ref(false), infoOpen = ref(false);
let copyTimer = null;

const encrypt = () => {
  if (!textarea.value) { ElNotification({ title: "提示", message: "请输入需要加/解密的内容", type: "warning", zIndex: 99999 }); return; }
  try {
    const bytes = CryptoJS.DES.encrypt(textarea.value, secretKey.value);
    if (bytes.ciphertext.sigBytes && bytes.ciphertext.words) result.value = bytes.toString();
    else ElNotification({ title: "失败", message: "加密失败，请检查输入内容", type: "error", zIndex: 99999 });
  } catch (e) { ElNotification({ title: "失败", message: "加密失败", type: "error", zIndex: 99999 }); }
};
const decrypt = () => {
  if (!textarea.value) { ElNotification({ title: "提示", message: "请输入需要加/解密的内容", type: "warning", zIndex: 99999 }); return; }
  try {
    ...
    if (!result.value) ElNotification({ title: "失败", message: "解密失败，请检查密钥是否正确", type: "error", zIndex: 99999 });
  } catch (e) { ElNotification({ title: "失败", message: "解密失败", type: "error", zIndex: 99999 }); }
};
const clear = () => { secretKey.value = ""; textarea.value = ""; result.value = ""; };
const copyResult = () => {
  if (!result.value) { ElNotification({ title: "提示", message: "结果为空", type: "warning", zIndex: 99999 }); return; }
  navigator.clipboard.writeText(result.value).then(() => {
    copyDone.value = true; clearTimeout(copyTimer); copyTimer = setTimeout(() => copyDone.value = false, 2000);
    ElNotification({ title: "成功", message: "结果已复制到剪贴板", type: "success", zIndex: 99999 });
  }).catch(() => ElNotification({ title: "失败", message: "复制失败", type: "error", zIndex: 99999 }));
};
</script>
...
.pw-algo-badge { display: flex; align-items: center; justify-content: center; width: 52px; height: 52px; border-radius: 14px; background: rgba(34, 211, 238, 0.12); color: var(--theme-btn-hover-color); font-size: 15px; font-weight: 700; font-family: var(--mono-font-family); flex-shrink: 0; letter-spacing: .02em; }
...
.pw-key-row { display: flex; align-items: center; gap: 14px; margin-bottom: 22px; padding: 14px 18px; background: rgba(230, 237, 243, 0.03); border-radius: 12px; }
.pw-key-input-wrap { width: 200px; }
.pw-key-hint { font-size: 12px; color: var(--text-secondary); }
...
.pw-info-dl { margin: 14px 0 0; display: grid; grid-template-columns: 1fr 1fr; gap: 6px 24px; div { display: flex; gap: 8px; padding: 6px 0; border-bottom: 1px solid var(--border-color); } dt { font-weight: 600; color: var(--color); white-space: nowrap; } dd { color: var(--text-secondary); margin: 0; } }
@media (max-width: 860px) { .pw-card { padding: 20px 16px; } .pw-card-head { flex-direction: column; align-items: flex-start; gap: 12px; } .pw-io-grid { flex-direction: column; gap: 12px; } .pw-io-arrow { display: none; } .pw-key-row { flex-direction: column; align-items: stretch; gap: 8px; } .pw-key-input-wrap { width: 100%; } .pw-toolbar { flex-wrap: wrap; } .pw-info-dl { grid-template-columns: 1fr; } }
</style>
