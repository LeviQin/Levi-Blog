...
.pw-result-label { width: 75px; font-size: 12px; font-weight: 600; color: var(--text-secondary); flex-shrink: 0; text-align: right; }
.pw-result-input { flex: 1; :deep(input) { font-family: var(--mono-font-family); font-size: 13px; } }
.pw-copy-sm { display: flex; align-items: center; justify-content: center; padding: 0; border: none; background: transparent; color: var(--text-secondary); cursor: pointer; width: 32px; height: 32px; transition: color .15s; &:hover { color: var(--theme-btn-hover-color); } }
